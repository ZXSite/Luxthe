...
> 从 v1 到 v3.7.8 — 完整开发历程、架构演进、关键修复、核心优化与已知陷阱
...
v3.6.8 开发阶段曾针对部分 iOS Safari 环境中的移动端开关表现进行多次排查。最终版本统一使用标准 click 事件处理桌面端和移动端开关，并在 v3.6.9.2 补充夜间模式强调色。本节仅保留为历史调试记录。

v3.6.8 中曾尝试 6 次修复（SCSS 注释、touch-action、双事件监听等），全部失败并已回退。当时分析的可能原因：
...
仅此 3 行。主题自带：移动 header、紧凑搜索、归档三栏、文章 widget（分类→标签→TOC）、可点击页脚。
...
### 7.5 文档一致性修正与最终发布 (v3.7.8)

**背景**: v3.7.7 标签 (`3db987e`) 早于迁移文档和开发手册提交 — 下载 v3.7.7 得不到最新的文档修复。

**修复**:
- 发布新的 v3.7.8 标签（`2bb7773`），v3.7.7 保持不动
- 迁移文档版本号一律改为 `v3.6.8+`，移除具体版本绑定
- 桌面布局示意图拆为首页/文章页两图
- Hugo Module 配置路径从单一 `config/_default/module.toml` 扩展为 `hugo.toml` 或拆分配置
- 手册三处事实修正：移动端开关改为历史记录、文章 widget 顺序修正、发布步骤顺序修正
- 清理 6 处 Markdown 尾随空白
- 开发手册全文版本引用统一为 v3.7.8

**发布流程规范**:
```
# 正确顺序（v3.7.8 采用的流程）
git push origin main          # 1. 推送分支
等待 main CI 全部通过           # 2. 等待绿灯
git tag -a v3.7.8 -m "..."    # 3. 标注标签
git push origin v3.7.8         # 4. 推送标签
确认标签指向最终审核提交        # 5. 最终验证
```

**教训**: 标签一旦推送即不可移动（破坏他人本地 clone 的 `git fetch`）。文档修复应合并到主题仓库后打新标签，而非 force-push 旧标签。

### 7.6 v3.6.9.x–v3.7.8 关键数据

| 指标 | 数值 |
|------|------|
| 修复版本数 | 5 (v3.6.9.1 / .2 / .3 / v3.7.7 / v3.7.8) |
| 修改文件数 | 16 |
| 新增 CI 步骤 | 3 |
| 新增 i18n 键 | 3 (`list.tag`, `list.category`, `article.featuredImage`) |
| 清理尾随空白 | 8 处 |
| 文档修正 | 迁移指南 8 处 + 手册 5 处 |
...
**决策**: 已推送的 Git 标签永远不 force-push。文档修复通过打新标签发布，而非移动旧标签。

**理由**: 他人本地 clone 的 `git fetch` 默认不覆盖已有标签。force-push 标签会导致不同 clone 看到不同的提交，破坏构建可复现性。

**示例**: v3.7.7 → v3.7.8：迁移文档和手册修复合并到 main 后打新标签，v3.7.7 保持不动。
...
| `git tag -f` + `git push -f` | 标签与远程不一致 | 永远不打新标签移动旧标签，落后者打新标签 |
...
- [ ] 等待 main 分支 CI 全部通过
- [ ] `git tag -a vX.Y.Z -m "LuxTHE vX.Y.Z"`
